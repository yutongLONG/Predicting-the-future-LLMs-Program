...
def load_finance_data(logger):
    ...
    logger.info(f"Loaded finance data: {len(df_finance)} records")
    return df_finance
...
def get_candidate_finance_text(candidate_name, month, df_finance, candidates_seen_previously):
    ...
    if rank == 1: rank_text = "the highest among all candidates"
    elif rank == 2: rank_text = "the 2nd highest among all candidates"
    elif rank == 3: rank_text = "the 3rd highest among all candidates"
    else: rank_text = f"ranked {rank} out of {total_candidates} candidates"

    return (f"{candidate_name} has raised {format_money(total)} in total fundraising, "
            f"{rank_text}, and raised {format_money(quarter)} in the most recent quarter.")

def setup_logging(model_name, unique_id):
    log_dir = "logs"
    os.makedirs(log_dir, exist_ok=True)
    log_file = f"{log_dir}/v47_finance_fixed_{model_name}_{unique_id}.log"
    logging.basicConfig(
        level=logging.INFO,
...
def run_election_simulation(model, logger, validation_data, finance_data, temp_nomination, temp_polling, temp_betting):
    logger.info(f">>> STARTING 2024 ELECTION SIMULATION (FINANCE + CONFIDENCE) - Temps: nom={temp_nomination}, pol={temp_polling}, bet={temp_betting}")
    ...
    logger.info("--- TASK 1: PARTY NOMINATIONS (WITH CAMPAIGN FINANCE) ---")
    ...
    for month in MONTHS:
        for party in PARTIES:
            ...
            sys_prompt = (
                ...
                f"The current time period is {month} 2024. Use the provided campaign finance data to inform your predictions."
            )
            ...
            if month in ["January", "February"]:
                stem = (
                    ...
                    f"of becoming the {party} nominee.\n\n"
                    f"CAMPAIGN FINANCE DATA ({month} 2024):\n"
                    f"{finance_context}\n\n"
                    f"Provide percentages (0-100) for each candidate, considering fundraising momentum."
                )
                ...
                stem = (
                    ...
                    f"Some candidates may be gaining or losing momentum.\n\n"
                    f"CAMPAIGN FINANCE DATA ({month} 2024):\n"
                    f"{finance_context}\n\n"
                    f"Evaluate each candidate's nomination likelihood as percentages (0-100)."
                )
                ...
                stem = (
                    ...
                    f"become clearer in terms of front-runners.\n\n"
                    f"CAMPAIGN FINANCE DATA ({month} 2024):\n"
                    f"{finance_context}\n\n"
                    f"Assess current nomination likelihood for each candidate (0-100 percentages)."
                )
            else:
                stem = (
                    f"TIME CONTEXT: General election season ({month} 2024)\n\n"
                    f"The {party} nominee is likely determined by {month}. However, assess the "
                    f"final standing of each candidate in the nomination process.\n\n"
                    f"CAMPAIGN FINANCE DATA ({month} 2024):\n"
                    f"{finance_context}\n\n"
                    f"Provide percentages (0-100), considering fundraising data."
                )
    ...
    logger.info("\n--- TASK 2: NATIONAL POLLING & ELECTION (WITH FINANCE DATA) ---")
    ...
    for month in MONTHS:
        ...
        if month == "November":
            sys_prompt = (
                ...
                f"This is the final outcome, not a poll. Use campaign finance data as an indicator of strength."
            )
            stem = (
                ...
                f"CAMPAIGN FINANCE DATA ({month} 2024):\n"
                f"{finance_context}\n\n"
                f"Provide the final vote share percentages (0-100) for each party, considering fundraising."
            )
        else:
            sys_prompt = (
                f"You are predicting national polling for the 2024 US Presidential Election "
                f"as of {month} 2024. Use campaign finance data to inform predictions."
            )
            stem = (
    ...
    logger.info("\n--- TASK 3: PREDICTION MARKETS (WITH FINANCE DATA) ---")
    betting_results = []
    ...
    for date_str in betting_dates:
        ...
        sys_prompt = (
            ...
            f"Use campaign finance data as valuable information about candidate viability and momentum."
        )
        ...
        stem = (
            ...
            f"CAMPAIGN FINANCE DATA ({month_name} 2024):\n"
            f"{finance_context}\n\n"
            f"Provide win probabilities (0-100) for each candidate, using fundraising data."
        )
    ...
    try:
        ...
        for config_idx, (t_nom, t_pol, t_bet) in enumerate(temp_configs):
            ...
            df_poll, df_election, df_bet, df_nomination = run_election_simulation(
                model, logger, validation_data, finance_data,
            ...
            if not df_poll.empty:
                fname = os.path.join(model_output_dir, f"national_polls_results_{args.model_name}_finance_{temp_code}_{unique_id}.csv")
                df_poll.to_csv(fname, sep=";", index=False, encoding='utf-8-sig')
                logger.info(f"✓ Saved Polling: {fname}")

            if not df_election.empty:
                fname = os.path.join(model_output_dir, f"national_election_results_{args.model_name}_finance_{temp_code}_{unique_id}.csv")
                df_election.to_csv(fname, sep=";", index=False, encoding='utf-8-sig')
                logger.info(f"✓ Saved Election: {fname}")

            if not df_bet.empty:
                fname = os.path.join(model_output_dir, f"national_betting_odds_results_{args.model_name}_finance_{temp_code}_{unique_id}.csv")
                df_bet.to_csv(fname, sep=";", index=False, encoding='utf-8-sig')
                logger.info(f"✓ Saved Betting: {fname}")
                
            if not df_nomination.empty:
                fname = os.path.join(model_output_dir, f"nomination_confidence_{args.model_name}_finance_{temp_code}_{unique_id}.csv")
