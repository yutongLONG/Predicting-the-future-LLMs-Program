...
# --- Modified to take the specific temperatures ---
def run_election_simulation(model, logger, validation_data, timeline_by_month, temp_nomination, temp_polling, temp_betting):
    logger.info(f">>> STARTING 2024 ELECTION SIMULATION (TIMELINE + CONFIDENCE) - Temps: nom={temp_nomination}, pol={temp_polling}, bet={temp_betting}")
    ...
    logger.info("--- TASK 1: PARTY NOMINATIONS (WITH TIMELINE) ---")
    ...
    for month in MONTHS:
        for party in PARTIES:
            ...
            # Build question stem with timeline ONLY
            if month in ["January", "February"]:
                ...
                stem += (
                    ...
                    f"4. Debates, endorsements, controversies affect standing\n\n"
                ...
                stem += (
                    ...
                    f"{timeline_events}\n\n"
                    f"INSTRUCTIONS:\n"
                    f"1. Campaign suspensions (shown in events) = <5% support\n"
                    f"2. Primary wins indicate strong voter support\n"
                    f"3. Front-runners typically consolidate support by this phase\n\n"
                    f"Provide nomination likelihood percentages (0-100) for each candidate."
                )
                ...
                stem += (
                    ...
                    f"2. Campaign suspensions = zero viability (<5%)\n"
                    f"3. Convention events (if mentioned) indicate nominee selection\n\n"
                    f"Provide nomination likelihood percentages (0-100) for each candidate."
                )
            ...
            sys_prompt = (
                ...
                f"The current time period is {month} 2024. Use campaign timeline events to make informed predictions."
            )
            ...
            # --- Confidence check with temp_nomination ---
    ...
    logger.info("\n--- TASK 2: NATIONAL POLLING & ELECTION (WITH TIMELINE) ---")
    ...
    for month in MONTHS:
        ...
        if month == "November":
            sys_prompt = (
                ...
                f"This is the final outcome, not a poll. Use campaign events to predict actual vote shares."
            )
            stem = (
            ...
            stem += (
                ...
                f"2. Use all available information: events, polls\n"
            ...
            sys_prompt = (
                ...
                f"as of {month} 2024. Use campaign events to inform predictions."
            )
            stem = (
            ...
            stem += (
                ...
                f"2. Consider typical polling patterns for {month}\n\n"
        ...
        # --- Confidence check with temp_polling ---
    ...
    logger.info("\n--- TASK 3: PREDICTION MARKETS (WITH TIMELINE) ---")
    betting_results = []
    ...
    for date_str in betting_dates:
        ...
        stem += (
            ...
            f"2. Suspended campaigns = near-zero probability\n"
            f"3. Market probabilities reflect overall win likelihood\n\n"
        ...
        sys_prompt = (
            ...
            f"Use campaign events to inform market probability estimates."
        )
        ...
        # --- Confidence check with temp_betting ---
    ...
    try:
        ...
        for config_idx, (t_nom, t_pol, t_bet) in enumerate(temp_configs):
            ...
            df_poll, df_election, df_bet, df_nomination = run_election_simulation(
                model, logger, validation_data, timeline_by_month,
            ...
            if not df_poll.empty:
                fname = os.path.join(model_output_dir, f"national_polls_results_{args.model_name}_timeline_{temp_code}_{unique_id}.csv")
                df_poll.to_csv(fname, sep=";", index=False, encoding='utf-8-sig')
                logger.info(f"✓ Saved Polling: {fname}")

            if not df_election.empty:
                fname = os.path.join(model_output_dir, f"national_election_results_{args.model_name}_timeline_{temp_code}_{unique_id}.csv")
                df_election.to_csv(fname, sep=";", index=False, encoding='utf-8-sig')
                logger.info(f"✓ Saved Election: {fname}")

            if not df_bet.empty:
                fname = os.path.join(model_output_dir, f"national_betting_odds_results_{args.model_name}_timeline_{temp_code}_{unique_id}.csv")
                df_bet.to_csv(fname, sep=";", index=False, encoding='utf-8-sig')
                logger.info(f"✓ Saved Betting: {fname}")
                
            if not df_nomination.empty:
                fname = os.path.join(model_output_dir, f"nomination_confidence_{args.model_name}_timeline_{temp_code}_{unique_id}.csv")
